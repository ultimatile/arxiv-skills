...
One case is exempt, and all four of its conditions hold together: the id was given without a version, a source is cached on disk, `.arxiv-fetch.json` already records a later revision of the same paper, and the record that answered was DataCite's. DataCite lists a new revision a few hours after arXiv announces it, so its record can trail what arXiv serves; the fetch step then keeps the recorded revision, deletes nothing, and the conversion runs on it. What the frontmatter's `version` then names is stated under `version` in `references/output-format.md`, and the fetch step prints a note on stderr naming both revisions. When arXiv itself answered, the revision it names is authoritative and a later recorded one is replaced.
...
To resolve, re-run `convert-paper` with `--tex-file` pointing at the correct main file (whether the re-run reuses the already-downloaded source is described under Re-download of cached files):
