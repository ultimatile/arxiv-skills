...
def test_absent_doi_journal_render_as_bare_null_keys():
    # A record with no DOI must emit `doi:` (null), distinct from omitting the
    # key. A bare `key:` line, not `key: ""`, is what a parser reads as None.
...
def test_the_document_does_not_name_which_source_answered():
    # Both sources render the same key set, and none of its keys names the
    # source. A reader that had to know which record answered would need the
    # converter to say what a null implies about the paper, which is the
    # bibliographic question it does not answer.
    ...
    assert set(from_arxiv) == FRONTMATTER_KEYS
    assert set(from_datacite) == FRONTMATTER_KEYS
    assert "metadata_source" not in FRONTMATTER_KEYS
    for parsed in (from_arxiv, from_datacite):
        assert arxiv_metadata.METADATA_SOURCE_ARXIV not in parsed.values()
        assert arxiv_metadata.METADATA_SOURCE_DATACITE not in parsed.values()
...
# --- lookup: the feed has to identify the paper before it is read -----------

_ATOM_ANOTHER_PAPER = _ATOM_ENTRY.replace(b"2606.09995v2", b"2606.09996v2")
_ATOM_UNPARSEABLE_ID = _ATOM_ENTRY.replace(
    b"<id>http://arxiv.org/abs/2606.09995v2</id>", b"<id>http://[unclosed</id>"
)
_ATOM_WITHOUT_AN_ID = _ATOM_ENTRY.replace(
    b"    <id>http://arxiv.org/abs/2606.09995v2</id>\n", b""
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            _ATOM_ANOTHER_PAPER,
            "2606.09995",
            "arXiv answered with a record for 2606.09996, not 2606.09995",
        ),
        (
            _ATOM_ENTRY,
            "2606.09995v1",
            "arXiv answered with 2606.09995v2, not the requested 2606.09995v1",
        ),
        (
            _ATOM_UNPARSEABLE_ID,
            "2606.09995",
            "arXiv's entry id names no revision: [unclosed",
        ),
        (
            _ATOM_WITHOUT_AN_ID,
            "2606.09995",
            "arXiv's entry carries no id to identify the paper by",
        ),
    ],
...
def test_a_feed_naming_another_paper_leaves_the_fallback_to_answer(transport):
    # The other half of the rule above: the refusal is what lets DataCite run,
    # so the paper still gets a record rather than the run recording none.
    requested = transport(_record("2409.03108"), arxiv=_ATOM_ANOTHER_PAPER)
    result = fetch_metadata("2409.03108")
...
def test_an_entry_id_the_url_parser_rejects_still_identifies_the_paper(transport):
    ...
    # urlparse raise. The tail split recovers an id that does name this
    # paper's revision, so the record is read rather than refused — the
    # identity check tests what the id says, not how it parsed.
    feed = _ATOM_ENTRY.replace(
        b"<id>http://arxiv.org/abs/2606.09995v2</id>",
        b"<id>http://[::1/abs/2606.09995v2</id>",
    )
...
def test_the_journal_key_is_null_on_a_record_that_carries_no_such_field(transport):
    # Only arXiv's feed has a journal reference. DataCite's registration has no
    # such field, so the key is null on its records — which the document states
    # as a property of the answering record, not of the paper.
    transport(_record("2409.03108"))
    result = fetch_metadata("2409.03108")
    assert result.metadata is not None
    assert result.metadata.journal is None
...
def test_an_atom_title_is_not_decoded_twice(transport):
    # The XML parser has already decoded the entities, so running the prose
    # decoder over them again would turn a title that spells "&gt;" literally
    # into ">".
    feed = _ATOM_ENTRY.replace(
        b"<title>A Study of Things</title>",
        b"<title>Writing &amp;gt; in a title</title>",
    )
    transport(b"unused", arxiv=feed)
    result = fetch_metadata("2606.09995")
    assert result.metadata is not None
    assert result.metadata.title == "Writing &gt; in a title"
...
def test_the_arxiv_request_drops_a_legacy_subject_class(transport):
    ...
    # The entry comes back spelled the way arXiv spells it, which is also the
    # spelling the identity check has to compare against.
    feed = _ATOM_ENTRY.replace(b"2606.09995v2", b"math/0309136v2")
    requested = transport(b"unused", arxiv=feed)
    result = fetch_metadata("math.GT/0309136")

    assert requested == [arxiv_metadata._ARXIV_API_URL + "?id_list=math%2F0309136"]
    assert result.status == METADATA_OK
    assert result.metadata is not None
    assert result.metadata.version == "math/0309136v2"
...
"""
    transport(_http_error(404), arxiv=feed)
    result = fetch_metadata("2606.09995")
    assert result.failure_cause.startswith("arXiv: arXiv reported an error for this id")
...
def test_the_lookup_takes_exactly_one_positional_argument():
    ...
    positional = [
        parameter
        for parameter in inspect.signature(fetch_metadata).parameters.values()
        if parameter.kind
...
def test_the_warning_names_no_single_source_as_the_one_that_failed():
    # Two sources are asked, and the cause already says what each of them
    # said. A message naming one of them would contradict the cause beside it
    # whenever the other was the one that answered.
    text = format_unavailable_warning("2606.09995", cause="arXiv: x; DataCite: y")
    assert "arXiv record" not in text
    assert "DataCite record" not in text
...
def test_a_populated_record_does_not_imply_a_record_was_read():
    # The PDF path builds a record from the PDF's own title when the lookup
    # failed. A present record is not evidence that either source was reached.
...
def test_a_document_with_no_arxiv_id_cannot_claim_a_record_was_sought(status):
    ...
    # read" nor "the lookup failed" describes the document. Only
